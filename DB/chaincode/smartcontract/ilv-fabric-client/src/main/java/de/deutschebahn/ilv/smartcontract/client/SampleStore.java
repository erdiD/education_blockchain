...
public class SampleStore {
    ...
    public SampleUser getMember(String name, String org) {
        ...
        // Try to get the de.deutschebahn.ilv.smartcontract.managment.de.deutschebahn.ilv.smartcontract.client.SampleUser state from the cache
        SampleUser sampleUser = members.get(SampleUser.toKeyValStoreName(name, org));
        if (null != sampleUser) {
            return sampleUser;
        }

        // Create the de.deutschebahn.ilv.smartcontract.managment.de.deutschebahn.ilv.smartcontract.client.SampleUser and try to restore it's state from the key value store (if found).
